...
- **Two version readers could be answered by a comment** (`template/scripts/src/common.dart.jinja`, `template/scripts/src/frb_pins.dart`) — both read a file as text with an unanchored pattern and `firstMatch`, which takes whichever match comes first in the file rather than the declaration. `getUpstreamVersion` is the one that mattered: a commented-out `<crate> = { … tag = "…" }` — the shape an upgrade leaves behind when the old line is kept for reference — outranked the live pin below it. Measured in a generated project, not reasoned about: a stale comment above the real pin read the stale tag, and a comment naming a *newer* tag read that one. The second direction is the dangerous one, because this feeds `check_updates.dart` — the update check then reports the dependency as already current, and an upstream release, security fixes included, silently never lands. Nothing fails; the update just never happens.

  `frbVersionFromGeneratedBindings` had the same shape, and was the only reader in `frb_pins.dart` that was not anchored; the other five are, on purpose. Its input is generated, so a stray earlier mention is unlikely rather than impossible — and it is the one way that gate could report agreement it had not checked.

  Both patterns are anchored to the start of a line now, and the inline table may no longer span lines. The upstream rule is split out of the disk read as `parseUpstreamTag`, the way `frb_pins.dart` already splits every reader from the disk, so it is testable without a fixture tree. That is what made `test/scripts/common_test.dart` templated — its fixtures name the upstream crate, which is an answer. The file contained no jinja syntax, so the `.jinja` rename is the whole of the change to the part that was already there, and the rendered path is unchanged.

- **`make release` verified only that a release with the right version *string* existed** (`template/scripts/src/release.dart.jinja`) — the crate version does not move until stage 1 runs, so running stage 2 on its own finds whatever release the previous cut left behind, passes the check, and publishes the current bindings against that binary. Neither runtime net catches it: `rustContentHash` compares the FFI *surface*, which an FRB upgrade need not move, and the codegen assert compares the bindings against the flutter_rust_bridge *runtime package*, never against the native library. What differs is the generator that decided the argument marshalling, so the failure is a wire mismatch inside a consumer's app.

  The preflight now also reads `frb_generated.dart` and `rust/Cargo.toml` **at the tag** and refuses when the codegen version — or, on projects that have upstream crates, the upstream pin — differs from the working tree. Over the GitHub API rather than by fetching the tag, because over-fetching the tag namespace is exactly what v3.0.3 narrowed the release fetch to avoid. It fails closed when it cannot read them, and `--skip-frb-check`, whose warning now names both checks rather than only the existence one, remains the escape hatch. The comparison is a pure function (`describeFrbReleaseDrift`) and is covered in both directions.

- **The README claimed one iOS architecture where the build workflow builds two** (`template/README.md.jinja`) — `build-<package>.yml` builds `aarch64-apple-ios`, `aarch64-apple-ios-sim` **and** `x86_64-apple-ios`, and the release carries an `ios-simulator-x86_64` archive, while the platform table said `arm64`. Every other column in that table already lists all of the architectures actually shipped.
...
- **`make run-example-web`** (`template/Makefile.jinja`) — builds the wasm module, wipes `example/web/pkg/`, then starts the example app in Chrome. The order is the content: without the build the browser gets whatever wasm was compiled last, which can be another commit's Rust; without the wipe a stale copy can be served and the failure reads as a Rust bug. It deliberately does not pass `--wasm` — that compiles the *Dart* half with dart2wasm, which flutter_rust_bridge's generated decoders do not support.

  It also closes a gap in the "run everything through make" rule: bringing the example up in a browser was the one routine task with no target, so it was done by hand with a `flutter run` nobody wrote down.

- **The FRB skill documents the newtype a browser handle needs** (`template/.claude/skills/frb-patterns/SKILL.md.jinja`) — FRB requires `Send + Sync`, and anything wrapping a `JsValue` is neither, so an opaque type holding a `web_sys` object fails to compile against a bound nobody wrote. The fix is a newtype with `unsafe impl Send + Sync` whose safety argument is the target itself — `wasm32-unknown-unknown` is single-threaded — and the section says so, including that the same code on a native target would be unsound rather than a workaround.

- **A test that refuses to let `template/` name one generated project** (`scripts/test_no_project_names.py`) — greps the whole rendered surface for the upstream libraries and the projects wrapping them, and fails with file, line and the offending text. It runs in `make test` alongside the release-script tests, and it was mutation-checked: reintroducing the `libsignal` this release removes turns it red and naming the file it is in, removing it again turns it green.

  The list of names is deliberately a list rather than a rule — add one when a new project is generated. A false positive costs one commit that has to reach for a variable; a miss ships the wrong library's name to everybody.

- **A generated project refuses a pull request whose FRB bindings were never regenerated** (`template/.github/workflows/codegen-guard.yml`) — two lines of shell that fail any pull request carrying the `codegen-failed` label that `check-<native>-updates.yml` attaches when `make codegen` does not complete. It exists because the alternative was measured and lost. Replaying the AI reviewer over all 47 merged pull requests in a generated project put its recall on this exact condition at 6 of 28 — 21% — even though the label sits in plain text in the context file the reviewer reads, and even though its reasoning on the ones it did catch was sound. A model that understands a rule and applies it one time in five is not a gate; the same rule stated directly is 100%, at no cost.

  The condition is the label alone, deliberately: when codegen fails an unchanged `lib/src/rust/` is the *expected* state, so an "and the bindings did not change" clause would add nothing and would let the check pass on a pull request where somebody hand-edited generated output — the worse of the two outcomes. `labeled` and `unlabeled` are in the trigger list so that removing the label after a manual fix re-runs the check rather than leaving the pull request red with nothing left to fix, and the labels are read back from the API rather than from the event payload, because the label is attached in the same breath as the pull request is opened. The file is copied verbatim rather than rendered, like `ai-review.yml` and `repair-build.yml`, so the two copies stay byte-identical. Twenty-eight pull requests had merged carrying this label in the project where it was measured.

- **A gate that actually executes web code** (`template/Makefile.jinja`, `template/rust/Cargo.toml.jinja`, `template/rust/src/utils.rs.jinja`, `template/.github/workflows/test-reusable.yml.jinja`) — `make test-web` runs the crate's `#[wasm_bindgen_test]` tests in a real headless browser through wasm-pack. Until now a generated project had no way to run web code at all: `make test` is the Dart VM and `make build-web` only compiles, so every `cfg(target_arch = "wasm32")` branch was covered by nothing. Those branches are exactly the ones nothing else can reach — a wasm32 body is a *different implementation* of the same function rather than the same code on another host, so a green native suite is no evidence about it.

  The scaffold gains the one test that proves the chain works, against the one function that already has two implementations: `current_time()` reads the system clock natively and calls `js_sys::Date::now()` on wasm32. It asserts the clock is past 2020, which is what separates "the call reached JavaScript and came back" from a stub or a zero. Verified in a generated project, both directions: the test passes in headless Chrome 152, and mutating the wasm32 body to `let millis = 0u64` fails it with `implausible browser clock: 0 ms` and exit 2. A test nobody has watched fail is not evidence of anything, which is why the mutation is recorded here rather than the pass alone.

  `wasm-bindgen-test` is declared under `[target.'cfg(target_arch = "wasm32")'.dev-dependencies]`, so a native `cargo test` does not build a browser harness it can never run. It is invisible to `make third-party-notices` and does not need to be there: that generator runs `cargo tree --edges normal,build`, which excludes every dev-dependency on every target.

  ⚠ **Pinning is about the unpinned download, not about a mismatch.** wasm-pack looks for a chromedriver on `$PATH`, honours `--chromedriver`, and otherwise DOWNLOADS THE LATEST — a network fetch on every run. The `CHROMEDRIVER` *environment* variable is not honoured, because wasm-pack overwrites it with its own, hence a make variable that becomes the flag. The CI step resolves the runner image's preinstalled driver and fails closed rather than letting wasm-pack fetch one. And it pins the DRIVER, never the BROWSER: with no `webdriver.json` and no `WASM_BINDGEN_TEST_WEBDRIVER_JSON` the runner sends empty capabilities and ChromeDriver launches whatever Chrome is installed. The verifying run printed `Not found` under "Try find webdriver.json", which is the honest reading of every green run that does not set it.

  `WASM_BINDGEN_TEST_TIMEOUT` is raised from the runner's 20 s default in the same file. The browser runs every test on one JS thread, so a test that blocks it starves the callbacks every concurrently-driven test is waiting on, and the resulting failure names whichever test was scheduled LAST rather than the slow one — it reads as a hang somewhere unrelated. Raising it before the first slow test is cheaper than reading that log.

- **`make rust-doc`, and a CI gate on it** (`template/Makefile.jinja`, `template/.github/workflows/test-reusable.yml.jinja`) — rustdoc over the hand-written crate, under `RUSTDOCFLAGS=-D warnings` so a broken intra-doc link fails the build instead of printing a warning nobody reads. It covers the half `make doc` structurally cannot see: the `[...]` links in the parts of `rust/src/` that stay in Rust — private items, `//!` module headers, anything not copied out to Dart.

  ⚠ **`--document-private-items` is why the number means anything, and this was measured in a generated project rather than reasoned about.** rustdoc resolves intra-doc links only in the items it is DOCUMENTING, and the scaffold's `mod utils;` is private. With a deliberately broken link in it: without the flag, `cargo doc` exits **0** and reports **zero** unresolved links; with the flag, exit **101** and the error. So the same target either gates or reports 0 over a surface it never opened, depending on one flag — and the zero reads as evidence, which is worse than having no target at all.

  The wasm32 leg is gated on `enable_web`, because `rustup target add wasm32-unknown-unknown` lives in `setup-web`: running it unconditionally would fail every `enable_web=false` project on a missing target. Both legs run with `cd rust` rather than `--manifest-path`, because cargo discovers `.cargo/config.toml` relative to the CWD and invoking from the repository root silently drops `rust/.cargo/config.toml`.

- **A BLOCKING documentation gate** (`template/dartdoc_options.yaml.jinja`, `template/.pubignore.jinja`, `template/.github/workflows/test.yml.jinja`, `template/.github/workflows/test-reusable.yml.jinja`) — `make doc` was in the Makefile and in no workflow, and it was a generator: `dart doc` exits 0 on unresolved references. `dartdoc_options.yaml` promotes `unresolved-doc-reference` to an error and CI runs the target, so it now fails the build.

  It earns its place in *this* template rather than in a project that happens to want strict docs. The Dart surface under `lib/src/rust/` is generated from the Rust docstrings in `rust/src/api/`, copied verbatim — so a reference in Rust's intra-doc syntax is valid Rust, resolves on the Rust side, and reaches pub.dev as a dead link that nothing else looks at.

  Verified in a generated project in both directions, because a gate that has only been seen passing has not been seen working: the untouched scaffold reports 0 warnings and 0 errors, and an injected `[ThisSymbolDoesNotExistAnywhere]` in `lib/src/` gives `error: unresolved doc reference` and exit 2. The file is NOT gated on `enable_web`, so the clean run was confirmed on both branches — a gate that reddened every `enable_web=false` project on day one would be a poor trade for the drift it prevents. ⚠ What it cannot catch: plain backticks resolve nothing and so never warn. It proves every `[...]` reference points at a real symbol, not that the prose names the right one.

  The file is `.pubignore`d, and the reason travels with it: pub.dev runs dartdoc itself and would honour the same promotion, so shipping it would let a future dartdoc release break documentation generation for an already-published version, which can never be re-cut.

- **The Web target is compiled and tested by CI at all** (`template/.github/workflows/test-reusable.yml.jinja`) — the workflow had **zero** web steps. `make build-web` existed as a target and ran nowhere, so a change that broke the wasm32 build could reach a release tag with every CI leg green; `make build` is host-only and cannot see it. `setup-web`, `build-web` and the browser suite now run on the Linux x86_64 leg, gated on `enable_web`.

- **`make rust-geiger`** (`template/Makefile.jinja`) — cargo-geiger's census of memory-unsafe expressions across the dependency graph. DIAGNOSTIC, deliberately wired into no workflow, and the comment says why rather than leaving it to be rediscovered: cargo-geiger 0.13 **exits non-zero on a clean run** (it ends with `error: Found N warnings` where every one is `Dependency file was never scanned:` for a non-Rust file — READMEs, `.proto`, a `.c`), so a gate would have to filter that before its exit code meant anything; and the count is not the project's to hold still, since the FRB-generated bridge dominates it and every `make codegen` rewrites that file. No setup target installs it, which the comment also says.

### Changed

- **The bookkeeping gates run last** (`template/.github/workflows/test-reusable.yml.jinja`) — `verify-third-party-notices` and `verify-frb-pins` ran before `analyze`, `format-check` and the test suite. A failing step takes every step after it with it, and the notice inventory is derived from the resolved dependency graph, so it goes stale on **every** dependency bump — which is exactly the pull request whose code most needs testing. The result was that a bookkeeping mismatch meant the Dart suite never ran at all on that leg. Both checks now run after the tests and the coverage step: they still fail the job, they just no longer decide whether anything else reports.

- **Dependabot stops proposing a native-hooks major the SDK cannot resolve** (`template/.github/dependabot.yml.jinja`) — `hooks` >= 2.1.0 pulls `record_use ^1.0.0`, which needs `meta ^1.19.0`, while the pinned Flutter pins `meta: 1.17.0` exactly; the first stable SDK to relax that is 3.47.0. The bump cannot be merged and cannot be fixed in the repository either, and the damage is not local: the root package resolves fine, so it breaks `example/` and every Flutter consumer on an older SDK while CI looks merely red. `code_assets` moves with it.

  Written as `update-types: ["version-update:semver-major"]` rather than `versions: ["2.x"]`, and that is the interesting half. This template already shipped a wildcard ignore once, for a GitHub Action, and Dependabot parsed it through `Gem::Requirement`, which has no wildcard expansion — the entry matched nothing while looking exactly like protection. `update-types` needs no version string parsed at all.

- **The upstream-bump checklist gains the four things that went wrong in practice** (`template/.claude/skills/update-{{ package_name }}/SKILL.md.jinja`) — each of these cost a real bump, and none of them was written down anywhere:

  **A feature gate reads as a removal.** `E0599: no variant named X` looks exactly like a deletion, and upstream notes tend to say "gated" rather than "removed"; the wrong diagnosis costs a wrapper rewritten to live without something still present. The step now says to check the upstream feature table first, and that features are not transitive: enabling one on the top-level crate does nothing for a sibling crate this project depends on directly.

  **Cargo features are additive and they ship.** A feature enabled anywhere — including under a `cfg(target_arch = "wasm32")` table — is enabled everywhere the crate is built; there is no "tests only". It matters most for features that change what error *values* contain, because those need no panic to reach a caller: the string crosses FFI on the ordinary error channel and lands in the consuming application's logs. The check is `strings` over the built artifacts, native and wasm, rather than a reading of the manifest.

  **Advisory ignores need re-validating, not carrying forward.** A bump moves the dependency graph the justification was written against. Empty both lists, re-run the audits, re-add only what fires again — an entry matching nothing is reported as `advisory-not-detected` and should be deleted rather than kept in case the crate returns.

  **An MSRV bump moves two files.** `rust-version` in `rust/Cargo.toml` is what CI reads, but it is rendered from the `rust_version` answer, and so is the toolchain version README.md advertises. Raise the answer with the manifest, or the README promises a toolchain that can no longer build the crate and the next `copier update` renders the stale number back over it.

- **`[profile.release]` says why it has no `panic` key** (`template/rust/Cargo.toml.jinja`) — adding `panic = "abort"` trades a small binary saving for skipping unwinding, and unwinding is what runs `Drop`: every zeroize-on-Drop in a crate would silently stop happening on the one path where it matters most, a failure while key material is live. Nothing fails; the wipe just does not occur. The comment also records the two limits of that statement — `-Cpanic=abort` in RUSTFLAGS bypasses the profile entirely, and `wasm32-unknown-unknown` aborts on panic by target default, so on the web this is a SECURITY.md limitation rather than something a profile key can fix.

- **CONTRIBUTING is a document rather than a stub** (`template/CONTRIBUTING.md.jinja`) — it went from a four-line "Getting Started" straight into the development workflow, so everything between "I have a fork" and "I can run the gates" was missing. It now carries prerequisites (with the note that a consumer needs none of them), a fork-and-clone that clones the fork rather than the upstream it used to hand over, what `make setup` actually installs, how to get `make` on Windows, the project layout with the two conventions that are load-bearing rather than cosmetic, branch and contribution guidance, when and why to regenerate the bindings, a pull-request checklist, and a compact Makefile reference that points at `make help` for the full list rather than duplicating it into something that goes stale.

- **Release binaries are built with `--locked`** (`template/.github/workflows/build-{{ package_name }}.yml.jinja`, `template/Makefile.jinja`) — the release workflow ran `cargo build --release` without it on every native target — the ten in the build matrix, the separate Windows job, and the Android builds through cargo-ndk — so the versions resolved in `rust/Cargo.lock` were not provably the versions in the artifact consumers download. `cargo install cargo-ndk` and `cargo install cargo-audit` gained it too. The cargo-ndk **version** is still unpinned and is now flagged in place, since that job produces the shipped Android `.so` files.

  ⚠ **`make build` deliberately does NOT get the flag, and that asymmetry is measured rather than assumed.** A project generated from this template has no `rust/Cargo.lock` until something builds it for the first time — copier's `_tasks` do not create one, and generating it during rendering is not an option, since it would resolve the upstream git dependencies over the network against whatever `native_repo` was answered. In a fresh render, `cargo build --locked` exits **101** with `the lock file ... needs to be updated but --locked was passed to prevent this`; with a lock present it exits 0. `test-reusable.yml` runs `make build` on all four of its matrix legs on every push, so the flag there would fail a brand-new project's very first CI run on every one of them. The reproducibility claim belongs where the shipped artifact is produced, and that is where the flag is; the Makefile comment says so, and says to add it once the lockfile is committed.

  The cargo-ndk form was checked rather than assumed to forward: `cargo ndk -t <target> --platform <n> build --release --locked` on cargo-ndk 4.1.2 behaves identically to the same command without the flag — same compilation, same downstream failure on a machine missing the Android target, and no `unexpected argument`. The flag reaches cargo.
...
- **The `pub` and `cargo` groups take minor and patch only** (`template/.github/dependabot.yml.jinja`) — grouping a major with everything else blocks the rest: one unmergeable entry takes the whole pull request down, and the only action left is to close it.

  It separates more than its name suggests, and the first run after the change is the evidence. Dependabot's commit trailers report a 0.x bump as `version-update:semver-minor` — `rand` 0.9 → 0.10 and `rusqlite` 0.34 → 0.40 both say so — but its *grouping* applies Cargo's own reading, where a 0.x minor is the breaking bump, and excludes them from a `minor`+`patch` group anyway. So the six-crate cargo group split into a group of two (`log` patch, `uuid` 1.x minor) plus individual pull requests for `rand`, `sha2`, `hkdf` and `aes-gcm-siv`; in the other generated project, `zeroize` 1.8 → 1.9 stayed in the group and `rusqlite` came out alone. Exactly the intended shape, and it is the version string rather than the trailer that decides.

- **`dart-lang/setup-dart` moves to 1.8.1, with the matcher that broke it switched off** (`template/.github/actions/setup-fvm/action.yml.jinja`) — 1.8.0 added a problem matcher for `dart analyze` and registers it with `::add-matcher::dart-analyzer.json`, resolving that path against `GITHUB_ACTION_PATH`. For an action invoked from inside another composite action that variable holds the *caller's* directory, so the runner looks for the file under `.github/actions/setup-fvm/`, does not find it, and fails the job seconds in — before anything is built. Reproduced on all four platforms in every generated project, and `publish.yml` and `build-libsignal.yml` call the same action, so a release would hit it too.

  Upstream ([dart-lang/setup-dart#198](https://github.com/dart-lang/setup-dart/issues/198)) is open and 1.8.1 does not fix it, so the SHA pin advances together with `problem-matcher: 'false'` rather than ahead of it: that input only exists from 1.8.0 on, so neither half is safe on its own. The action's own test is `problemMatcher != 'false'` against the literal string, hence the quotes. Nothing in `setup-fvm` runs `dart analyze` — that job uses the FVM-pinned SDK — so the matcher was buying a generated project nothing even when it worked.

- **The Dependabot ignore that stood in for it is deleted, because it never stood for anything** (`template/.github/dependabot.yml.jinja`) — an earlier commit in this same unreleased range pinned setup-dart back by ignoring `versions: ["1.8.x"]`. Dependabot parses a `github-actions` ignore through `Gem::Requirement`, which has no wildcard expansion: `"1.8.x"` becomes `= 1.8.x`, and 1.7.2, 1.8.0, 1.8.1 and 1.9.0 all fail it. It was measured rather than reasoned about — the bot went on rebuilding its branch over the very commit that added the entry, with 1.8.1 still in the diff. Shipping it would have handed three generated projects an entry that reads as protection and enforces nothing; the fix belongs in the action, where it now is.

- **`make coverage` measures the code somebody wrote** (`template/Makefile.jinja`) — the ignore covered `frb_generated*` only, while `flutter_rust_bridge.yaml` puts *all* of codegen's output under `lib/src/rust/`; the rest of that directory is the same generator one layer up, and what goes uncovered in it is almost all `hashCode` and `operator ==` on value classes. So a `make codegen` run could move a project's coverage badge with nobody having written a line — measured in a generated project, where a growing API surface took it from 100% to 93.8% and 41 of the 45 missing lines were exactly that boilerplate. With the directory excluded the number is coverage of hand-written code, which is the only thing a project can act on.

  The comment records why the second glob is not written `**/lib/src/rust/**`, because that is the form somebody will later "normalise" it to: a glob starting with `**` can never match an absolute path (`glob`'s `DoubleStarNode` is `canMatchAbsolute = false`), so it is tested only against the CWD-relative path, in which nothing precedes `lib`. It matches nothing and switches the ignore off silently. Measured across seven glob forms before picking this one.

- **`lints` starts a generated project on 6.1.0** (`template/pubspec.yaml.jinja`) — the window slides one minor and keeps its width, so the cap it exists for is unchanged; what changes is that a project generated today no longer opens with a Dependabot pull request for a minor that is already known good. It is known good rather than assumed: 6.1.0 runs green under `make analyze ARGS="--fatal-infos"` in two generated projects independently, one of them across the full four-platform matrix. The per-project ratchet the comment describes is untouched — this only moves where a new project starts on it.

- **A generated project starts on flutter_rust_bridge 2.13.0** (`copier.yml`) — and unlike the `lints` window above, the default is the *only* thing that decides this. `flutter_rust_bridge` is in the generated Dependabot ignore list on purpose, because moving it is not a one-file bump: the version has to move in five places at once — the pubspec constraint, `frb_version`, `FRB_CODEGEN_VERSION`, the `=` pin in `rust/Cargo.toml`, and the committed bindings, which have to be regenerated by that exact codegen because the runtime asserts its own version equals the one recorded in them. So a project generated on 2.12.0 gets no pull request offering 2.13.0; it simply stays there until somebody performs the whole coordinated move by hand.

  2.13.0 is known good rather than assumed. The full pipeline was run in a generated project — `make setup-frb-codegen`, `make codegen`, `make build`, 811 tests, and `make build-web` — and the change it produces is mechanical: the Dart diff is the `@generated by` stamp in 17 files plus one `codegenVersion` string, `rustContentHash` does not move, and the generated Rust changes only in how `Result::Ok` is spelled, in whitespace, in the `@generated by` stamp and codegen-version const, and in gaining `mismatched_lifetime_syntaxes` in its allow list. Every changed line was classified against those categories with none left over, and none of them is semantic. The wasm leg matters most and is the one a host build never covers: `make build` is host-only and CI compiles wasm only on a `libsignal_frb-*` tag, so 2.13.0's Web target had never been compiled anywhere before this.

- **The repair agent is told what it is explaining, and to leave generated files alone** (`template/.github/agent-prompts/repair-build.md`) — a second repair pass in a generated project lost its diagnosis. A new flutter_rust_bridge landed between the two runs, the repair job's own checkout began failing for an unrelated and newer reason, and that local reproduction displaced the recorded log as the thing being explained. The agent was not what broke; the invariant was simply never written down. The prompt now states it — the subject is the failure in the log of the run named in `failure.env`, anything met locally that does not match it is a second finding for `notes` rather than `cause`, and a failure that does not reproduce is a `cannot-fix` naming the recorded failure rather than a licence to adopt some other one as the thing to fix.

  It also forbids editing generated output by hand, which is what that pass produced: a single `codegenVersion` line under `lib/src/rust/`, reverted by the next `make codegen`. That is a red build silenced rather than repaired, and catching it in review means knowing what the generator will do rather than reading the diff. The path allowlist cannot express this rule, which is why it belongs in the prompt: those paths have to stay writable, because running `make codegen` — already permitted — is exactly how a legitimate fix reaches them. The rule is about how a file may change rather than which file it is, and every file it covers already opens by saying "please do not edit it".

- **freezed is an answer rather than a fact** (`copier.yml`, `template/pubspec.yaml.jinja`) — `freezed_annotation`, `freezed` and `build_runner` were generated unconditionally, but flutter_rust_bridge needs them only when the Rust API has data-carrying enums or structs it renders as sealed classes. Neither project generated from this template has such an API: both strip the three dependencies by hand, and both do it again after every `copier update`, because the template keeps putting them back. `enable_freezed` defaults to true, so a new project is generated exactly as before and nothing changes for one that wants them; a project that does not can now say so once instead of every release.

- **The `lints` comment says what to do when the cap is reached, and what Dependabot will do first** (`template/pubspec.yaml.jinja`) — it said "widen the upper bound there once it is green", which reads as permission to delete the cap the rest of the comment exists to justify. It now says to move the window — both bounds, one minor — which is what the template itself did.

  The second half is the part nobody had written down: `versioning-strategy: increase-if-necessary` leaves a constraint alone only while it still admits the new version. Once the new minor is outside the cap there is nothing to leave alone, and Dependabot writes its own idiomatic form, which is a caret. Measured in a generated project on the 6.0 → 6.1 bump: the strategy was already active when the pull request was created, and the constraint it proposed was `^6.1.0`; the bot regenerated that branch a day later and proposed the caret again. So a capped pin at its ceiling always comes back uncapped, and narrowing it again is the reviewer's job rather than something the strategy prevents.

- **The advisory ignore list says when an entry should be deleted** (`template/rust/deny.toml.jinja`) — the preamble explained when to add one and nothing else. It now also says that an entry cargo-deny reports as `advisory-not-detected` is a signal to remove it rather than to keep it in case the crate returns — an ignore nobody can trigger is indistinguishable from one quietly covering a second advisory it was never reviewed for — and that this list is not interchangeable with `.cargo/audit.toml`, because cargo-deny also reports unmaintained advisories, which `cargo audit` does not fail on.

- **SECURITY.md hands the reporter a link instead of asking them to find an address** (`template/SECURITY.md.jinja`, `template/README.md.jinja`) — "Email the maintainers directly or use GitHub's private vulnerability reporting feature" required a generated project to publish contact details that the scaffolding never provided, and the README's "**Note:** Update SECURITY.md with your contact information" was the reminder to do it. The policy now points at `https://github.com/<repo>/security/advisories/new`, which needs no contact details at all and works for outside reporters once the repository has the feature switched on — so it names that step too. The README note goes with it: it now asks for something the file no longer needs.

- **The README prerequisites say who they are for** (`template/README.md.jinja`) — the list sits under "## Development" but reads as the cost of using the package, which is the opposite of what the build hook exists for: a consumer needs no Rust toolchain, no FVM and no `make`, because `flutter build` downloads a precompiled library. The section now says so before listing what a contributor needs, and the Rust entry points at `rust-version` in `rust/Cargo.toml` rather than standing as an independent claim about the floor.

### Fixed

- **`template/` named one particular generated project in four places** (`template/.github/workflows/codegen-guard.yml`, `template/.claude/skills/release-frb-crate/SKILL.md.jinja`, `template/.github/actions/setup-fvm/action.yml.jinja`, `template/test/scripts/update_changelog_test.dart.jinja`) — every one of them was prose sitting next to code that used the variables correctly: a comment, a heading sentence, a test's explanation. Nothing renders a comment, nothing compiles it, and no generated project's CI has a reason to complain, so they survived.

  The expensive one was in `codegen-guard.yml`, which has no `.jinja` suffix and therefore no substitution at all — its `::error::` line is the only sentence a human reads when that gate fires, and in three of four projects it named a library they do not have. It is now worded neutrally rather than templated: the file is full of `${{ github.token }}`, and giving it a `.jinja` suffix would mean escaping every one of those as `${{"{{"}}`, which is a larger change with more ways to go wrong than the sentence deserved. The other three take the variable that was already in scope one line away.

- **A rate-limited GitHub reply no longer becomes "no release notes were published"** (`template/scripts/src/update_changelog.dart.jinja`) — `curl -s` carries no `-f`, so it exits 0 on 403, 429 and every 5xx, and the only other guard matched the single exact string `Not Found`. A rate-limited reply reads "API rate limit exceeded for <ip>", missed both tests, fell through to an absent `body`, and returned the "nothing was published" sentence — an API failure laundered into a fact the changelog was then written from, in a script whose stated rule is that nothing is guessed and the entry is left unwritten. The request is unauthenticated, so the ceiling is 60 an hour per IP and GitHub's runners share IPs; this is an ordinary Tuesday rather than a corner case.

  The decision now lives in `releaseNotesFrom`, split out of the fetch so it can be tested without a network, and keys on `tag_name` rather than `body` because a real release with an empty body is the ordinary case here and had to keep working. `_fetchUpstreamCommits` next door already tested for `commits` for the same reason and was never affected — the correct shape was thirty lines below the broken one. Six tests cover it, and the three failure cases all fail without the guard.

- **The upstream badge showed a version nobody released, and then the tag instead of the version** (`template/README.md.jinja`, `template/scripts/src/check_updates.dart.jinja`, `template/test/scripts/check_updates_test.dart.jinja`) — two defects in one line, and the second hid behind the first. The badge was rendered with a hardcoded `-v1.0.0-`, so a generated project opened its README advertising a version its upstream had never published; the first automatic bump then overwrote it with `checkResult.latestVersion`, which is the raw tag.

  For a project whose `version_tag_prefix` is a bare `v` the raw tag happens to read correctly, which is why this survived. For one whose prefix is not — `openmls-v`, say — it renders the prefix twice: `badge/openmls-openmls-v0.9.0-orange.svg`, because the badge text already carries the library name. The tag is the right value for the three other substitutions the same function performs (the `tag = ` in `rust/Cargo.toml`, the CLAUDE.md example, `upstream_version` in the answers file), so the fix is not to normalise `newVersion` — it is to normalise in the one place that needs it. That place is now `updateReadmeBadge`, extracted from `updateVersionFiles` so it can be tested without a filesystem, and it preserves whatever leading `v` the badge already had. The rendered badge follows the same rule from the `upstream_version` answer, sliced rather than `replace`d, because `replace` would cut the prefix string wherever it occurred rather than only at the front.

  Verified by rendering: a project answering `openmls-v0.9.0` now opens with `badge/openmls-v0.9.0-orange.svg` and `updateReadmeBadge` writes the same string on the next bump. Four new tests cover it, including one that only a project with a non-default prefix can fail.

- **`make release-frb` left half of a wrapped highlight behind** (`template/scripts/src/release_frb.dart.jinja`, `template/test/scripts/release_frb_test.dart.jinja`) — the companion to the insert branch fixed earlier in this same unreleased range, and the more expensive of the two. Replacing an existing `**<crate> vX.Y.Z**` highlight assigned over the bullet's first line only. A highlight somebody had extended onto a second, indented line kept that line, now orphaned under a stamp that no longer mentions it — and since the stamp is the last thing to touch `[Unreleased]` before the release closes it, the orphan lands in a released section, which this template's own changelog contract says is immutable.

  The replacement now takes the whole bullet: everything up to the next bullet or heading, and across a blank line only when what follows is indented, which is how a highlight carrying a second paragraph is written. An unindented line with no blank line before it is a lazy continuation and goes too; an ordinary paragraph after a blank line stays. Three tests, one per case.

- **`cargo deny` reported this template's own licence list** (`template/rust/deny.toml.jinja`) — `allow` opened with `"{{ license }}"` above a static list that already contains MIT, Apache-2.0 and BSD-3-Clause: three of the four choices `copier.yml` offers, so three of four generated projects listed their licence twice and cargo-deny flagged the duplicate against this file rather than against anything in the dependency tree. AGPL-3.0 is the one choice that genuinely adds entries and it already had its own branch. The list is now built once and de-duplicated, so adding a licence to it cannot reintroduce the collision.

- **`git clone` followed by a `cd` into a directory that does not exist** (`template/README.md.jinja`) — the Setup block cloned `{{ github_repo }}` and then ran `cd {{ package_name }}`. Those are the same word only when the repository is named exactly like the package; for a project called `openmls` living in `openmls_dart` the second line fails, as the first thing a new contributor runs. It now cds into what `git clone` actually creates. In the same file, the `{% endif %}` closing the web build block printed its own newline inside a fenced code block, so every web-enabled project rendered a stray blank line before the closing fence.

- **The documented `frb_version` default was two errors out of date** (`README.md`, `CLAUDE.md`, `.claude/skills/create-project/SKILL.md`, `.claude/skills/test-template/SKILL.md`) — six lines advertised `^2.12.0`, which was wrong twice over: the version had moved to 2.13.0, and the caret contradicts the answer's own contract. `copier.yml` asks for the version "exact, no range operator" and its validator strips a leading `^`, so a documented `^` teaches the one form the prompt exists to discourage — and the `--data frb_version="^2.12.0"` examples handed it to anyone copying a command. Nothing gated on them: the root `make test` runs the release-script tests only, and nothing outside `template/` reads `copier.yml` defaults, so the drift was invisible to CI and had to be read to be found.
...
[Unreleased]: https://github.com/djx-y-z/copier-dart-frb-wrapper/compare/v4.7.0...HEAD
