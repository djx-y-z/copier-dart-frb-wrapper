...
- **`dart-lang/setup-dart` 1.8.x is ignored, temporarily and narrowly** (`template/.github/dependabot.yml.jinja`) — 1.8.0 added a problem matcher for `dart analyze` and registers it with `::add-matcher::dart-analyzer.json`. That path resolves against the *calling* action's directory, and the generated project calls setup-dart from inside its own `setup-fvm` composite action, so the runner looks for the file under `.github/actions/setup-fvm/`, does not find it, and fails the job fourteen seconds in — before anything is built. Reproduced on all four platforms in both generated projects.

  Only the 1.8 line is ignored, so 1.9.0 will arrive for evaluation rather than this freezing the action; 1.8.1, the latest at the time of writing, does not fix it. Worth knowing what the bump is worth here: setup-dart exists in that action solely to provide a `dart` binary for the next line, `dart pub global activate fvm`. Everything that builds and tests a generated project comes from the FVM-pinned SDK, so the bump moves no version that reaches the code — the feature it adds is inline `dart analyze` annotations on a path that does not run `dart analyze`.
